...
!======================================================
       ...
       if (rank == 0) then
          print *, "eval_projection_J; main_iter =", iter
       end if
...
!======================================================
       ...
       END IF
       if (rank == 0) then
          print *, "tau_brack(1)=",tau_brack(1),"tau_brack(2)=", tau_brack(2)
       end if
       if (rank==0) then
...
!=================================================
      SUBROUTINE projection_RCG(myfield, gradJ, gradJ_pre, dJ, dJ_actual, norm2, norm2_pre, beta_, RCG_flag, restart_flag_)
        ...
        ! temp2_solver_cx = (1+|D|^2)^(-s)exp(-2sigma)|D|^2 v
        call G_l_s_sigma_fourier(temp1_solver_cx, temp2_solver_cx, l, -s, -2.0_pr*sigma)
        ...
        ! temp1_solver = |D|^2 v/norm
        call fftbwd_m(temp1_solver_cx, temp1_solver, 3)
        
        ! temp1_solver_cx = (1+|D|^2)^(-s/2)exp(-sigma|D|)|D|^2 v/norm
        ! temp1_solver_cx = half n_v
        call G_l_s_sigma_fourier(temp1_solver_cx, temp1_solver_cx, l, -0.5_pr*s, -sigma)

        ! temp2_solver = (1+|D|^2)^(-s/2)exp(-sigma|D|)|D|^2 v/norm
        ! temp2_solver = half n_v
        call fftbwd_m(temp1_solver_cx, temp2_solver, 3)
        ...
        ! project gradJ_pre
        ! val = <gradJ_pre, (1+|D|^2)^(-s/2)exp(-sigma|D|)|D|^2 v/norm>_{L^2}
        call L2_product(gradJ_pre, temp2_solver, val)

        ! gradJ_pre = gradJ_pre - <gradJ_pre, (1+|D|^2)^(-s/2)exp(-sigma|D|)|D|^2 v/norm>_{L^2}*((1+|D|^2)^(-s/2)exp(-sigma|D|)|D|^2 v/norm)
        ...
        if (RCG_flag == 1) then
           ...
           temp2_solver = gradJ - (sqrt(3.0_pr)/2.0_pr)*gradJ_pre
           call L2_product(temp2_solver, gradJ, val)
           
           beta_ = val/norm2_pre

           ! dJ = P_{T_n}G^n + Beta_n * Gamma_{t_{n-1}d^{n-1}}(d^{n-1})
           dJ = gradJ + beta_*(sqrt(3.0_pr)/2.0_pr)*dJ
        end if
...
! Compute the cost function J = (||u(T)||_dotH^1)^2
...
!==========================================================
    SUBROUTINE report_J(myfield,tau_brack, count, mydt)
      ...
      call projection(myfield, gradJ_opt, gradJ_opt, norm2)
      CALL MPI_BARRIER(MPI_COMM_WORLD,Statinfo)
      ...
      do i = 0,count
         ...
         CALL MPI_BARRIER(MPI_COMM_WORLD,Statinfo)
         J = compute_J(Uvec, fix_dt1, 0, i,  1)
