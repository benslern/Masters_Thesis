...
\qquad In mathematics, Fourier analysis is the study of the way general functions may be represented or approximated by sums of simpler trigonometric functions. We make use of several different components of Fourier Analysis.
...
\end{figure}
The fourier transform of a derivative $u'(x)$ just has a term added to it,
$$F(u'(x))(k) = 2\pi i k F(u(x))(k)$$
...
\qquad A Fourier series is an expansion of a periodic function $f(x)$ into a sum of trigonometric functions. The formula for a Fourier series is,
$$f(x) = \frac{a_0}{2}+
\sum_{n=1}^\infty \left[ a_n\cos(n\pi x/L)+b_n\sin(n\pi x/L) \right]$$
where $a_n$ and $b_n$ are the Fourier Coefficients given by the formulas,
$$a_0 = \frac{1}{L}\int f(x)dx$$
$$a_n = \frac{1}{L}\int_{2L}f(x)\cos(n\pi x/L)dx$$
$$b_n = \frac{1}{L}\int_{2L}f(x)\sin(n\pi x/L) dx$$
...
imposed to ensure that $u$ is real. Therefore, the fourier expansion has the property that,
$$\overline{u(x)} = u(x)$$
If needed, the Fourier coefficients $\hat{u}_k$ can be computed explicitly via the integral,
$$\hat{u}_k = \frac{1}{(2\pi)^3}\int_{\Omega}e^{-ikx}u(x)dx.$$
\qquad The Fourier coefficients represent the amplitude and phase at each frequency of the series. From the Payley-Wiener theorem, if $u(x)$ is real analytic, then its Fourier coefficients decay exponentially and conversely, if a Fourier expansion has exponentially decreasing coefficients then it is real analytic.
...
suppose u and v are complex valued functions on R of period $2\pi$ that are square integrable (with respect to the Lebesgue measure) over intervals of period length
...
\qquad Functions can be thought of as points in an infinite dimensional space of vectors. We must first define some set operations.
The product of two sets is the set of pairs,
...
$$G^\sigma := \{v\in H^s(\Omega):\|v\|_{G^\sigma}:= <v,v>_{G^\sigma}^{1/2}<\infty\}, s\geq 3, \sigma > 0$$
with the inner product
$$\forall v,u\in G^\sigma, <v,u>_{G^\sigma} := \sum_{j\in \mathbb{Z}^3} (1+|2\pi j|^2)^s e^{4\pi\sigma |j|}\hat{v}_j \cdot \overline{\hat{u}}_j)$$
$$ = \int_\Omega (1+|D|^2)^s e^{2\sigma |D|}v\cdot u dx$$
where overbar denotes complex conjugation, where as the operators $|D|$ and $e^{\sigma|D|}$ are defined via
$$\left[ \hat{|D|v} \right]_j := 2\pi|j|\hat{v}_j$$
$$\left[ \hat{e^{\sigma|D|}v} \right]_j := e^{2\pi\sigma |j|}\hat{v}_j.$$
...
\subsection{leapfrog bk}
\subsection{ssprk3 bk}
...
$$\partial_t\textbf{u}+\textbf{u}\cdot \nabla \textbf{u}+\nabla p - \mu \Delta \textbf{u} = 0 \textbf{ in }\Omega\times (0,T]$$
$$\nabla \cdot \textbf{u} = 0 \text{ in }\Omega\times[0,T]$$
$$\textbf{u}(0) = \textbf{u}_0$$
where $\textbf{u}:[0,T]\times\Omega \rightarrow \mathbb{R}^3$ and $p:[0,T]\times\Omega \rightarrow \mathbb{R}$ are the velocity and pressure fields. $\mu >0$ is the coefficient of kinematic viscosity, $\textbf{u}_0$ is a divergence-free initial condition whereas the fluid density $\rho$ is assumed constant and equal to 1.
...
THIS SECTION IS COPIED FROM MY THESIS AND IS MY ORIGINAL WORK.
\\~\\
The Euler equations are the inviscid incompressible from of the Navier-stokes equations. They are defined by the equations
$$\partial_t\textbf{u}+(\textbf{u}\cdot\nabla)\textbf{u}+\nabla p = 0$$
$$\nabla \cdot \textbf{u} = 0$$
$$\textbf{u}(\textbf{x},0) = \textbf{u}_0(\textbf{x}).$$
...
An $\alpha-$energy equality was proved to hold for solutions for the Euler-Voigt equations for all $t\in \mathbb{R}$,
$$E_\alpha(t) := \|\textbf{u}^\alpha(t)\|_{L^2}^2 + \alpha^2\|\nabla \textbf{u}^\alpha (t)\|_{L^2}^2 = E_\alpha (0).$$
...
\qquad There are several different criteria that are used to determine whether a flow has formed a singularity. These criteria are described below.
...
$$\varepsilon(u(t)):=\frac{1}{2}\int_{\Omega}|\omega(t,x)|^2dx = \frac{1}{2}\|\nabla u(t)\|_{L^2(\Omega)}$$
where $\omega$ is again the vorticity.
...
$$\kappa(\epsilon) = \frac{\epsilon^{-1}\left[ \Phi(\eta + \epsilon\eta') - \Phi(\eta) \right]}{\langle \nabla\Phi(\eta),\eta' \rangle}. $$
This formula shows the gradient, round off, and truncation errors as a function of epsilon. The round off errors should be approximately of order $O(\epsilon^-1)$. Therefore, we expect the value of $\kappa$ do be dominated by round off errors for small $\epsilon$ values. The truncation errors should be of order $O(\epsilon^p)$ for some $p\geq 0$. Therefore, we expect the value of $\kappa$ to be dominated by the truncation errors for large values of $\epsilon$. Between these two regions the value of $\kappa$ will be determined by the gradient errors, which are themselves a product of the spatial and temporal resolution. With smaller values of $\Delta x$ and $\Delta t$ causing the gradient error to decrease, while also decreasing the range of $\epsilon$ over which the gradient errors dominate. Therefore, we expect the plot of $\kappa$ vs $\epsilon$ to be of the general form shown in the graph below, taken from Adjoint-based enforcement of state constraints in PDE optimization problems.

\begin{figure}[!ht]
\centering
\includegraphics[scale=0.6]{kappa_test.png}
\end{figure}
...
\begin{figure}[!ht]
\centering
\includegraphics[scale=0.3]{kappa_test_init_1_t1_result.png}
\end{figure}
\begin{figure}[!ht]
\centering
\includegraphics[scale=0.3]{kappa_test_init_1_t3_result.png}
\end{figure}
\begin{figure}[!ht]
\centering
\includegraphics[scale=0.3]{kappa_test_init_2_t1_result.png}
\end{figure}
\begin{figure}[!ht]
\centering
\includegraphics[scale=0.3]{kappa_test_init_2_t3_result.png}
\end{figure}
\begin{figure}[!ht]
\centering
\includegraphics[scale=0.3]{kappa_test_init_3_t1_result.png}
\end{figure}
\begin{figure}[!ht]
\centering
\includegraphics[scale=0.3]{kappa_test_init_3_t3_result.png}
\end{figure}
...
\begin{center}
\begin{large}
\textbf{Singularites in Fluid Mechanics}
\\~\\
\end{large}
\textbf{H.K. Moffatt}
\\~\\
\end{center}
This paper describes the types of singularities (mathematical or physical), the properties of singularities and the conditions under which they can form. As well as the ways to prevent the formation of singularities in some cases. Finally it shows that singularity formation still occurs despite viscocity in some fluid flows.
\\~\\
"Singularities of the Navier-Stokes equations occur when some derivative of the velocity
field is infinite at any point of a field of flow (or, in an evolving flow, becomes infinite
at any point within a finite time)."
\\~\\
The clay millennium prize questions if any initialy smooth velocity field of finite energy in an incompressible fluid becomes singular at finite time under Navier-Stokes evolution. 
