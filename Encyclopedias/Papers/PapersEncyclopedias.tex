...
The Euler–Voigt equations were proposed as an inviscid regularization of the Euler equations.The Euler–Voigt equations are given by
$$-\alpha^2\partial_t\nabla^2\textbf{u}+\partial_t\textbf{u}+(\textbf{u}\cdot\nabla)\textbf{u}+\nabla p = 0$$
$$\nabla \cdot \textbf{u} = 0$$
$$\textbf{u}(\textbf{x},0) = \textbf{u}_0(\textbf{x}).$$
Here $\alpha>0$ is a regularization parameter having units of length. Note that the usual incompressible Euler equations are formally obtained by setting $\alpha=0$.
