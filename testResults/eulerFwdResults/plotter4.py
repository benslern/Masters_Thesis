...
a = 8
for i in range(0,1):
    ax.set_title(r"Energy vs $t$ - $\alpha=$"+str(i)+r"$/1024$ - init $\|u_\alpha\|_{\dot{H}^1} = \sqrt{3}/2$")

    filename = "./alpha_"+str(i)+"_1024/resol_1024/energy_fwd_1.dat"
    Ts = []
    ase = []
    energy = []
    tot = []
    with open(filename, 'r') as file:
        for line in file:
            ...
            #tot.append(((i/1024)**2)*float(vals[5]) + float(vals[9]))
