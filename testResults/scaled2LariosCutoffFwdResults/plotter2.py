...
ax.set_title(r"Fig 3 inset - init $\|u_\alpha\|_{\dot{H}^1} = \pi$")
...
for i in range(0,1):
    ...
    for n in range(0,101,1):
        ...
        slopes.append(m)
    ax.plot(Ts[:len(slopes)],slopes,'-',ms=2,label=r"$1024\alpha$: "+str((r+i*4))+' - '+str((r+(i+1)*4)))
    print(slopes.index(min(slopes))*0.05)
