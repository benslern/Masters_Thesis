...
{\blue{\em   Given a smooth initial condition $\be$, does the Navier-Stokes (and Euler)
...
\left\{
\begin{aligned}
&-\alpha^2\partial_{t}\Delta\ua +\partial_{t}\ua+({\ua}\cdot{\bnabla}){\ua}
...
\item<2->
$\alpha > 0$ is the regularization parameter with units of length. Acts as a 'spectral filter'. The Euler-Voigt system is globally well-posed given smooth initial conditions [CITATION]. 
\medskip

\item<3->
As $\alpha\rightarrow 0$ the behaviour can be used to determine if the Euler equations will develop a finite-time singularity.
...
\begin{itemize}
\item<1-> Proven by Theorem 1 in [COMPUTATIONAL INVESTIGATION CITATION]. WHY DONT WE JUST GO DIRECTLY TO THEOREM 5.2 CONCLUSION
...
\begin{itemize}
\item<1-> Theorem 5.1 of [2nd COMPUTATIONAL INVESTIGATION CITATION]
...
\begin{itemize}
\item<1-> Modified from Theorem 5.2 [2nd COMPUTATIONAL INVESTIGATION CITATION]
...
\begin{subequations}
\begin{align*}
&\limsup_{\alpha\rightarrow 0}\left(\sup_{t\in[0,T^*]}\alpha^2\|\bn\ua(t)\|_{L^2}^2\right) = \\
&\limsup_{\alpha\rightarrow 0}\left(\|\bea\|_{L^2}^2+\alpha^2\|\bn(\bea)\|_{L^2}^2 - \inf_{t\in[0,T^*]} \|\ua(t)\|_{L^2}^2\right).
\end{align*}
\end{subequations}
We now make use of Theorem (\ref{thm:Theorem:1.2}), which demonstrates that the convergence of the solutions of the Euler-Voigt equations to the solution of the Euler equations is uniform on $[0,T^*]$. Therefore we can reduce the above equation to
\begin{equation*}
...
\begin{equation*}
\M_1 = \{\v \in V : \|\v\|_{\hdo} = C\}.
\end{equation*}

\item<3->
Euler and Euler-Voigt time scaling property
\begin{equation}
\tilde{\u}(\x,t) = \lambda \u(\x,\lambda t),
\end{equation}
...
\item<3->
A [CITATION] showed evidence for finite-time singularity in Euler from $\be_{TGV}$ at $T^* \approx 4$. Used $(\Omega = [0,2\pi]^3)$ and $V_0 = 1$. We use $\Omega = [0,1]^3$ and so scale down to $V_0 = 1/2\pi$.  
...
\item<1->
Verify $\|\v\|_{\hdo}$ with vorticity
\begin{equation*}
\|\omega\|_{L^\infty} = \|\bn\times \u\|_{L^\infty}
\end{equation*}
...
\item<3->
If $\alpha^2\pat(\tilde{\be}_{\alpha,T})>0$ as $\alpha \rightarrow 0$ then a singularity forms in Euler.
...
\item<1->
The first order finite difference equation for the G$\a$teaux differential is,
\begin{equation*}
...
Perturbations
\begin{equation*}
\ua=\ua+\upa, \bea=\bea+\bepa, \text{ and } p=p+p'
\end{equation*}
...
Momentum term
\begin{equation*}
\beta_n = \frac{\la P_{\T_n}\nabla\pat(\bea^{(n)}) - \Gamma_{\tau_{n-1}\d^{n-1}}P_{\T_{n-1}} \nabla\pat(\bea^{(n-1)}) \ra_{G^\sigma}}{\|P_{\T_{n-1}} \nabla\pat(\bea^{(n-1)})\|_{G^\sigma}^2}
\end{equation*}
