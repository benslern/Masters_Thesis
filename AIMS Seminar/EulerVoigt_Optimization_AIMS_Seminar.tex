...
\begin{itemize}
\item<1->
We modify ansatz 4.2 (Larios et al. 2018) 
$$\sup_{t\in[0,T^*]}\|\nabla\ua\|_{L^2}^2 \approx \mathcal{O}(\alpha^p)$$.
\medskip
...
\caption{Log-log plot of $max_{t\in[0,T^*]\|\nabla\ua\|_{L^2}^2}$ vs $\alpha$ at $T = 0, 0.1, ..., 5.0$}
\end{figure}
...
\begin{itemize}
\item<1->
Results of $\alpha^2\pat(\tilde{\be}_{\alpha,T})$ as $\alpha\rightarrow 0$.
...
\item<2->
Repeat with higher resolutions and expand to other initial conditions. Problem is non-convex.
